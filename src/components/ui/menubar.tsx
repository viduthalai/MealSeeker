  ...
  return (
    <div className="fixed bottom-0 left-0 z-50 w-full border-t bg-background ">
      <div className="grid grid-cols-5">
        <MobileMenuItem onClick={() => handleMenuClick('')} icon={<Home className="h-5 w-5" />} label="Home" active={active === 'member'} />
        ...
        <MobileMenuItem onClick={() => handleMenuClick('profile')} icon={<User className="h-5 w-5" />} label="Profile" active={active === 'profile'} />
      </div>
    </div>
  );
}
